...
| **4. Customer Pays via LUMO Pay** | `Order` (PAID), `PaymentTransaction`, `AuditLog`, `NotificationOutbox` | `$transaction` | `ORDER_PAID` | Customer, Supplier, Sales | **Customer UI**: Status timeline advances to "PAID". **Supplier UI**: Packing queue unlocked. | Unassigned Logistics/Agents |
...
| **6. Delivery Confirmation** | `Order` (DELIVERED/COMPLETED), `AuditLog`, `NotificationOutbox` | `$transaction` | `ORDER_DELIVERED` | Customer, Sales, Supplier | **Customer UI**: Prompted for review. **Supplier UI**: Settlement payout queued. | Unassigned Logistics |
