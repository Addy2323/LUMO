...
export default function AgentReportsPage() {
  ...
  // Dynamic telemetry based on hub orders
  const monthlyData = [
    { month: 'Jun', volumeUSD: Math.round(totalVolumeUSD * 0.1), inspections: Math.round(completedInspections * 0.2) },
    { month: 'Jul', volumeUSD: Math.round(totalVolumeUSD * 0.3), inspections: Math.round(completedInspections * 0.3) },
    { month: 'Aug', volumeUSD: Math.round(totalVolumeUSD * 0.6), inspections: Math.round(completedInspections * 0.5) },
  ]
    ...
    <div className="space-y-8">
      ...
      <Card className="bg-slate-900 border-slate-800">
        ...
        <CardContent className="p-6 space-y-4">
          {[
            { category: 'Mobile Accessories & Tech Hardware', pct: hubOrders.length > 0 ? 45 : 0, color: 'bg-brand-500' },
            { category: 'Solar Energy & Dual MPPT Inverters', pct: hubOrders.length > 0 ? 30 : 0, color: 'bg-emerald-500' },
            { category: 'Hotel Kitchenware & Commercial Utensils', pct: hubOrders.length > 0 ? 15 : 0, color: 'bg-purple-500' },
            { category: 'Garments, Textiles & Fabrics', pct: hubOrders.length > 0 ? 10 : 0, color: 'bg-blue-500' },
          ].map((c) => (
