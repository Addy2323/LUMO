...
export default function AdminPaymentsPage() {
  const [txns, setTxns] = useState<EscrowTransaction[]>([])
  ...
  const fetchDatabasePayments = async () => {
    ...
    try {
      ...
      if (result.data) {
        const parsedTxns: EscrowTransaction[] = []
        ...
        result.data.forEach((order: any) => {
          ...
          const gateway = order.payments?.[0]?.provider || order.paymentMethod || 'AzamPay'
          ...
          parsedTxns.push({
            ...
            gateway: gateway,
            escrowState: order.status === 'DELIVERED' ? 'Released to Supplier' : 'Held in Escrow Protection',
  ...
  function handleReleaseEscrow(id: string) {
    setTxns(
      txns.map((t) => (t.id === id ? { ...t, escrowState: 'Released to Supplier' } : t)),
    )
    toast.success('Admin Manual Escrow Release Executed!')
  }
  ...
  return (
    <div className="flex flex-col gap-6 font-sans antialiased text-foreground">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b pb-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">Database Master Escrow Payments &amp; Audits</h1>
          <p className="text-xs text-muted-foreground mt-1">
            Supervise platform mobile money &amp; card transactions, escrow holds, and payout releases directly connected to PostgreSQL.
          </p>
        </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base font-extrabold flex items-center gap-2">
              ...
              Escrow Transaction Ledger ({filtered.length})
            </CardTitle>
          ...
          <div className="divide-y border rounded-xl overflow-hidden">
            {loading ? (
              <div className="p-12 text-center text-xs text-muted-foreground">
                ...
                Loading live escrow ledgers from database...
              </div>
            ) : filtered.length === 0 ? (
              <div className="p-12 text-center text-xs text-muted-foreground space-y-1">
                <p className="font-semibold text-foreground">No escrow payment records found in database.</p>
                <p>Checkout payments will record in this escrow ledger automatically.</p>
              </div>
            ) : (
              filtered.map((t) => (
                <div key={t.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card hover:bg-muted/20 text-xs">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={t.escrowState === 'Released to Supplier' ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-white'}>
                        {t.escrowState}
                      </Badge>
                    </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-4">
                    ...
                    {t.escrowState !== 'Released to Supplier' && (
                      <Button size="sm" onClick={() => handleReleaseEscrow(t.id)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-8 text-xs">
